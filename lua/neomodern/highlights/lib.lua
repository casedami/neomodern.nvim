...
local function resolve_override(field, is_hex, new, palette)
	...
	elseif type(new) ~= "string" then
		vim.schedule(function()
			vim.notify(string.format("Neomodern: unknown value format for field '%s'", field), vim.log.levels.WARN)
		end)
		return nil
	end
	...
	vim.schedule(function()
		vim.notify(
			string.format("Neomodern: unknown color or color format -- '%s=%s'", field, new),
			vim.log.levels.WARN
		)
	end)
	return nil
